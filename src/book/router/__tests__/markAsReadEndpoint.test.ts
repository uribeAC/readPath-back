...
describe("Given the PATCH /books/mark-as-read/:bookId endpoint", () => {
  describe("When it receives a request with bookId of Dragon Ball Vol. 1 as to read", () => {
    test("Then it should respond with a 200 status code and Dragon Ball Vol. 1 book as read", async () => {
      ...
      const response = await request(app).patch(
        `/books/mark-as-read/${dragonBallId}`,
      );
      ...
      expect(body.book.state).toBe("read");
    });
  });

  describe("When it receives a request with bookId of Dragon Ball Vol. 1 as read", () => {
    test("Then it should respond with a 409 Book is already marked as read error", async () => {
      const expectedError = "Book is already marked as read";
